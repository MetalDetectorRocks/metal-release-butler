...
class MetalArchivesRestClientTest extends Specification {
  ...
  def "RestTemplate is called with correct URL"() {
    ...
    then:
    underTest.restOperations.getForEntity(UPCOMING_RELEASES_URL, _, _) >> ResponseEntity.ok(new MetalArchivesReleasesResponse(totalRecords: 0, data: []))
  }
  ...
  def "RestTemplate is called with correct response type"() {
    ...
    then:
    underTest.restOperations.getForEntity(_, MetalArchivesReleasesResponse, _) >> ResponseEntity.ok(new MetalArchivesReleasesResponse(totalRecords: 0, data: []))
  }
  ...
  def "RestTemplate is called with correct URL parameter range, starting with 0"() {
    ...
    then:
    underTest.restOperations.getForEntity(_, _, 0) >> ResponseEntity.ok(new MetalArchivesReleasesResponse(totalRecords: 0, data: []))
  }
